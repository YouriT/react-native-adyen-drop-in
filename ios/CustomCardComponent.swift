...
public final class CustomCardComponent: PaymentComponent, PresentableComponent {
    ...
    public init(paymentMethod: CardPaymentMethod, publicKey: String,buttonTitle:String) {
    ...
    private lazy var storedCardAlertManager: CustomStoredCardAlertManager? = {
        guard let paymentMethod = paymentMethod as? StoredCardPaymentMethod else {
            return nil
        }
        
        let manager = CustomStoredCardAlertManager(paymentMethod: paymentMethod, publicKey: publicKey, amount: payment?.amount)
    ...
    private lazy var formViewController: FormViewController = {
        ...
        formViewController.append(numberItem, using: CustomFormCardNumberItemView.self)
    ...
    private lazy var numberItem: CustomFormCardNumberItem = {
        CustomFormCardNumberItem(supportedCardTypes: supportedCardTypes, environment: environment)
    }()
    ...
    private lazy var footerItem: FormFooterItem = {
        ...
        if(self.buttonTitle!.count>0 ){
            footerItem.submitButtonTitle = self.buttonTitle
        }
